# Azure AKS ephemeral runner platform
...
| compute-16-vcpu-candidate | Standard_D16ads_v5 | 0-1 | ephemeral | One-at-a-time 16-vCPU qualification runner |
| compute-32-vcpu-density-candidate | Standard_F32s_v2 | 0-5 | ephemeral | Disabled density candidate |
...
only inside the interactive shell running Terraform:
    ...
    terraform -chdir=terraform/azure/runner-fleet init -reconfigure -backend-config=backend.hcl
...
Create ignored terraform/azure/runner-fleet/terraform.tfvars with the deployment
names, operator CIDRs, pod CIDR, service CIDR, and DNS service IP. Then run:

    terraform fmt -check -recursive terraform
    terraform -chdir=terraform/azure/bootstrap init -backend=false
    terraform -chdir=terraform/azure/bootstrap validate
    terraform -chdir=terraform/azure/runner-fleet init -backend=false
    terraform -chdir=terraform/azure/runner-fleet validate
...
digest:
    ...
    scripts/mirror-runner-image.sh copy ghcr.io/f5-sales-demo/self-hosted-runner@sha256:<digest> f5salesdemoarcca.azurecr.io/self-hosted-runner
