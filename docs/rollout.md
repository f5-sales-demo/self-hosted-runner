...
4. Use `scripts/arc-copy-pull-secret.sh arc-runner-cache arc/repositories/*.yaml` to reconcile the private GHCR credential into every ARC namespace. The source and every copied `ghcr-pull` secret must contain exactly `ghcr.io`; ACR pulls are anonymous. Deploy the provider compute scale set at 0-3 and the other approved compute scale sets at 0-2, then manually run ARC Compatibility before changing ordinary tests/native builds. Route release compilation last.
...
6. Burst two xcsh, two enriched-spec, and three provider compute jobs together. Confirm the shared pool never exceeds seven compute nodes, each repository stays at its exact cap, and excess demand queues cleanly. Then burst 30 socketless and 5 container-build jobs. Prove unique ephemeral pods, exact namespaces, no avoidable pending state, and node scale-down after 60 minutes.
