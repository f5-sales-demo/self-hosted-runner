...
4. Deploy `xcsh-compute` at 0-5, then manually run ARC Compatibility before changing ordinary tests/native builds. Route release compilation last.
...
6. Burst 30 socketless, 5 compute, and 5 container-build jobs. Prove unique ephemeral pods, exact namespaces, no avoidable pending state, and node scale-down after 60 minutes.
