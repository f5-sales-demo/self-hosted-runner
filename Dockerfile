...
RUN cc -O2 -Wall -Wextra -Werror /tmp/landlock-abi.c -o /usr/local/bin/landlock-abi \
    ...
    && chmod 0555 /usr/local/bin/landlock-abi /usr/local/bin/require-landlock-abi \
      /usr/local/bin/runner-entrypoint /usr/local/bin/runner-profile \
...
ENTRYPOINT ["/usr/local/bin/runner-entrypoint"]
LABEL org.opencontainers.image.source="https://github.com/f5-sales-demo/self-hosted-runner" \
      org.opencontainers.image.description="Ephemeral Docker-client runner for a private per-pod DinD daemon" \
