...
class AksArcContractTests(unittest.TestCase):
    ...
    def test_arc_artifacts_and_images_are_pinned(self) -> None:
        ...
        socketless = (ROOT / "arc/socketless-values.yaml").read_text(encoding="utf-8")
        ...
        for values in (socketless, build):
            self.assertIn("name: init-runner-runtime", values)
            self.assertIn("/usr/local/libexec/prepare-runner-tool-cache", values)
            self.assertIn("name: RUNNER_RUNTIME_DIR", values)
            self.assertIn("name: RUNNER_TOOL_CACHE", values)
            self.assertIn("name: HOME", values)
            self.assertIn("value: /runner-runtime/home", values)
            self.assertIn("readOnlyRootFilesystem: true", values)
            self.assertIn("name: runner-runtime", values)
            self.assertIn("mountPath: /runner-runtime", values)
            self.assertIn("mountPath: /home/runner", values)
            self.assertIn("mountPath: /tmp", values)
            self.assertIn("find /home/runner -mindepth 1 -maxdepth 1", values)
            self.assertIn("--target-directory=/runner-runtime", values)
        ...
        self.assertIn('--namespace "$cache_namespace"\n      --create-namespace', deploy)
